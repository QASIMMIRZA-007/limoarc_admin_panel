...
const ArchivedGroup = ({ item, refresh }) => {
  ...
  // const changePosition = async items => {
  //   dispatch(setLoader(true))
  //   try {
  //     const res = await ApiCall({
  //       route: `desease/sort_categories/${item?._id}`,
  //       token: token,
  //       verb: "put",
  ...
  const RestoreMedGroup = async () => {
    try {
      const res = await ApiCall({
        route: `desease/update_deleted_desease_status/${item?._id}`,
        token: token,
        verb: "put",
      })
      if (res?.status == 200) {
          ...
          <CardBody>
            ...
            >
              ...
              <div className="flex gap-[8px]">
                <h3
                  className="text-[#a18552] text-xs font-bold cursor-pointer"
                  onClick={() => {
                    RestoreMedGroup()
                  }}
                >
